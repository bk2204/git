...
----
  metadata-update = map-object-update
          shallow-update
          flush-pkt

  map-object-update = *map-object-line

  hash-algo        =  "sha1" / "sha256"
  map-object-type  =  "shallow" / "unshallow"
  map-object-line  =  PKT-LINE("map-object" SP hash-algo SP map-object-type
                               SP obj-id SP obj_id)
...
If the client and server both support the 'object-format-map' capability and
the client has requested one of the advertised hash algorithms, then the server
writes any 'map-object' lines.  These are required for every shallow or
unshallow that the server sends.  Unknown mapping types should be ignored.
...
The server will follow all 'map-object', 'shallow', or 'unshallow' lines with a
flush-pkt if any were sent.  If none of these directives were sent, the the
server omits the flush-pkt.

...
header. Most sections are sent only when the packfile is sent.
    ...
    output = acknowledgements flush-pkt |
	     [acknowledgments delim-pkt] [object-map-info delim-pkt]
			 [shallow-info delim-pkt] [wanted-refs delim-pkt]
    ...
    ack = PKT-LINE("ACK" SP obj-id LF)
		...
		object-map-info = PKT-LINE("object-map-info" LF)
		   *PKT-LINE(obj-map LF)
		obj-map = "map-object" SP algo SP kind SP obj-id SP obj-id
		algo = ("sha1" | "sha256")
		kind = ("shallow" | "unshallow" | "submodule" | "promisor" | "wanted-ref")
...
The server can advertise the `object-format-map` capability with a value `X`
(in the form `object-format-map=X`) to notify the client that the server is
able to map objects using hash algorithm X.  This allows the transmission of
information about shallows, objects from partial clones, and submodules in an
algorithm other than the default.  The server advertising this capability must
be capable of converting object IDs from any value of `object-format` it
supports to this algorithm.

...
static void flush(struct pack_ctx_reader *c)
{
	if (c->input_offset) {
		...
		git_hash_update(&c->input_ctx, c->input_buffer, c->input_offset);
			       ...
			       struct pack_ctx_reader *rdr)
{
	...
	do {
		...
		if (compat && compat_oid && type == OBJ_BLOB)
			git_hash_update(&c_compat, last_out, stream.next_out - last_out);
		if (buf == fixed_buf) {
			stream.next_out = buf;
	...
	if (compat && compat_oid) {
		if (type != OBJ_BLOB) {
			...
			git_hash_update(&c_compat, hdr, hdrlen);
			git_hash_update(&c_compat, outbuf.buf, outbuf.len);
				       ...
				       struct pack_ctx *c)
{
	...
	if (c->compat_algo) {
		 ...
		 * to disk because we may need it available when mapping other
		...
		repo_add_loose_object_map(the_repository->objects->sources,
					  &delta_obj->idx.oid,
					  &delta_obj->idx.compat_oid, LOOSE_TYPE_LOOSE);
		repo_unlock();
		strbuf_release(&outbuf);
	}
finish:
				...
				struct object_entry *obj)
{
	...
	void *data = NULL, *base = NULL, *cvtbuf, *delta = NULL, *to_free = NULL;
	...
	if (obj->type == OBJ_OFS_DELTA) {
		...
		if (!delta)
			die(_("cannot create delta for object %s"), oid_to_hex(&obj->idx.oid));
		append_obj_to_pack(f, obj, &obj->idx.oid, dheader + pos,
				   sizeof(dheader) - pos, delta, deltasz,
				   obj->size, obj->type, obj->real_type, 1);
	} else if (obj->type == OBJ_REF_DELTA) {
		if (repo_oid_to_algop(the_repository, &delta_base,
				      c->dest_algo, &delta_base))
			die(_("cannot convert delta base for object %s"),
			    oid_to_hex(&delta_base));
		delta = diff_delta(base, basesz, cvtbuf, cvtsz, &deltasz, 0);
				   ...
				   unsigned char *pack_hash)
{
	...
	*dest_pack = open_pack_file(&c, NULL);
	...
	QSORT(sorted_objects, nr_objects, compare_array_objects_by_compat_oid);
		  ...
		  unsigned char *hash)
{
	...
	if (!from_stdin) {
		...
		fsync_component_or_die(FSYNC_COMPONENT_PACK, c->output_fd, curr_pack_name);
		if (close(c->output_fd))
	...
	if (!from_stdin) {
		...
		write_in_full(1, c->input_buffer + c->input_offset, c->input_len);
	}

...
static int reset_refs(const char *rev, const struct object_id *oid)
{
	...
	struct object_id *orig, oid_orig, *old_orig, oid_old_orig;
	...
	if (!get_oid("HEAD", &oid_orig)) {
		...
		update_ref(msg.buf, "ORIG_HEAD", orig->hash, old_orig->hash,
			0, UPDATE_REFS_MSG_ON_ERR);
	} else if (old_orig)
		delete_ref("ORIG_HEAD", old_orig->hash, 0);
	set_reflog_message(&msg, "updating HEAD", rev);
	update_ref_status = update_ref(msg.buf, "HEAD", oid->hash, orig->hash,
				       0, UPDATE_REFS_MSG_ON_ERR);
	strbuf_release(&msg);

...
static void find_loose_object_map(struct repository *repo, struct strbuf *buf, int flags)
{
...
static int load_one_loose_object_map(struct repository *repo, struct object_directory *dir, int flags)
{
	...
	find_loose_object_map(repo, &path, flags);
...
int repo_write_loose_object_map(struct repository *repo, int flags)
{
	...
	find_loose_object_map(repo, &path, LOOSE_INDEX_LOOSE);

			...
			struct object_id *missing_oid)
{
	...
	switch (type) {
	case OBJ_TREE:
		ret = convert_tree_object(repo, &sbuf, repo->hash_algo,
		...
		*outbuf = strbuf_detach(&sbuf, outlen);
		...
		return 1;
	case OBJ_COMMIT:
		ret = convert_commit_object(repo, &sbuf, repo->hash_algo,
					    repo->compat_hash_algo, buf, len,
					    missing_oid);
		if (ret < 0)
			return ret;
		*outbuf = strbuf_detach(&sbuf, outlen);
		...
		return 1;
	case OBJ_TAG:
		ret = convert_tag_object(repo, &sbuf, repo->hash_algo,
					 repo->compat_hash_algo, buf, len,
					 missing_oid);
		if (ret < 0)
			return ret;
...
int write_object_file_compat(const void *buf, size_t len,
			     ...
			     const char *type, struct object_id *oid)
{
	char hdr[MAX_HEADER_LEN], compat_hdr[MAX_HEADER_LEN];
	int hdrlen = sizeof(hdr), compat_hdrlen = sizeof(compat_hdr);
	 ...
	 * it out into .git/objects/??/.
	 */
	write_object_file_prepare(r->hash_algo, buf, len, type, oid, hdr,
				  &hdrlen);
	if (compat_buf)
		write_object_file_prepare(r->compat_hash_algo, compat_buf,
					  compat_len, type, &compat_oid,
					  compat_hdr, &compat_hdrlen);
		      ...
		      struct object_id *oid)
{
	char hdr[MAX_HEADER_LEN];
	int hdrlen = sizeof(hdr);
	const void *compat_buf = NULL;
	size_t compat_len = 0;
	int to_free = 0, result;
	const struct git_hash_algo *compat_algo = the_repository->compat_hash_algo;
	...
	if (compat_algo) {
		...
		to_free = convert_object_file(the_repository, &compat_buf,
					      &compat_len, buf, len, typeval, NULL);
		if (to_free < 0)
			return error(_("failed to convert object file %s"), oid_to_hex(oid));
	}
	result = write_object_file_compat(buf, len, compat_buf, compat_len, type, oid);
	if (to_free)
		free((void *)compat_buf);
	return result;
}

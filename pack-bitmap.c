					  ...
					  int flags)
{
	...
	hash_pos = kh_put_oid(index->bitmaps, &stored->oid, &ret);
...
static int load_pack_bitmap(void)
{
	...
	bitmap_git.bitmaps = kh_init_oid();
...
static inline int bitmap_position_extended(const struct object_id *oid)
{
...
static inline int bitmap_position_packfile(const struct object_id *oid)
{
...
static int bitmap_position(const struct object_id *oid)
{
	int pos = bitmap_position_packfile(oid);
	return (pos >= 0) ? pos : bitmap_position_extended(oid);
}
			      ...
			      int bitmap_pos)
{
	...
	hash_pos = kh_get_oid(bitmap_git.bitmaps, oid);
...
static int should_include(struct commit *commit, void *_data)
{
	...
	bitmap_pos = bitmap_position(&commit->object.oid);
	if (bitmap_pos < 0)
		bitmap_pos = ext_index_add_object((struct object *)commit, NULL);

	if (!add_to_include_set(data, &commit->object.oid, bitmap_pos)) {
				   ...
				   struct bitmap *seen)
{
	...
	while (roots) {
		...
		if (object->type == OBJ_COMMIT) {
			khiter_t pos = kh_get_oid(bitmap_git.bitmaps, &object->oid);
	...
	while (roots) {
		...
		roots = roots->next;
		pos = bitmap_position(&object->oid);
...
void test_bitmap_walk(struct rev_info *revs)
{
	...
	pos = kh_get_oid(bitmap_git.bitmaps, &root->oid);
	...
	kh_foreach_value(bitmap_git.bitmaps, stored, {
		if (stored->flags & BITMAP_FLAG_REUSE) {
			if (!rebuild_bitmap(reposition,
					    ...
					    rebuild)) {
				hash_pos = kh_put_oid(reused_bitmaps,
						      &stored->oid,
						      &hash_ret);

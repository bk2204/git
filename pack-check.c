...

{
	...
	do {
		...
		offset += remaining;
		if (!pack_sig_ofs)
			pack_sig_ofs = p->pack_size - GIT_SHA1_RAWSZ;
		if (offset > pack_sig_ofs)
			remaining -= (unsigned int)(offset - pack_sig_ofs);
		git_SHA1_Update(&ctx, in, remaining);
	} while (offset < pack_sig_ofs);
	git_SHA1_Final(oid.hash, &ctx);
	pack_sig = GIT_HASH_TO_OBJECT(use_pack(p, w_curs, pack_sig_ofs, NULL));
	if (oidcmp(&oid, pack_sig))
		err = error("%s SHA1 checksum mismatch",
			    p->pack_name);
	if (hashcmp(index_base + index_size - GIT_SHA1_RAWSZ * 2, pack_sig->hash))
	...
	for (i = 0; i < nr_objects; i++) {
		entries[i].oid = nth_packed_object_oid(p, i);
		if (!entries[i].oid)

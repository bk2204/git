...
test_expect_success 'object with bad sha1' '
	...
	new=$(dirname $old)/$(test_translate -f oid ff-2) &&
...
test_expect_success 'unparseable tree object' '
	...
	junk=$(test_translate twenty-bytes-of-junk \
		twenty-bytes-of-junk-twelve-more) &&
...
test_expect_success 'tag pointing to nonexistent' '
	badoid=$(test_translate -f oid deadbeef) &&
	cat >invalid-tag <<-EOF &&
	object $badoid
	type commit
	tag invalid
...
test_expect_success 'alternate objects are correctly blamed' '
	...
	path=$(test_translate -f oid numeric) &&

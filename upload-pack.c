			     ...
			     const struct git_hash_algo *hash_algo)
{
	...
	struct strvec *rev_args = &pack_data->submodule_revision_args;
	...
	if (repo_has_accepted_promisor_remote(the_repository)) {
		...
		strvec_push(rev_args, "--missing=allow-promisor");
	}
...
static int send_one_mapped_submodule(const struct object_id *oid, void *unused UNUSED, void *data)
{
	/* The formatting is the same for both types of objects. */
	return send_one_mapped_shallow(oid, data);
}
				   ...
				   const struct git_hash_algo *map_hash_algo)
{
	...
	list_objects_filter_copy(&revs.filter, &data->filter_options);
	/*
	...
	setup_revisions_from_strvec(&data->submodule_revision_args, &revs, &s_r_opt);

	for (size_t i = 0; i < data->stdin_objects.nr; i++) {
		const char *line = data->stdin_objects.v[i], *p;
		int len = strlen(line);
		if (!len)
			break;
		if (*line == '-') {
